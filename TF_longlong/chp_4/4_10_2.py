...
x = tf.convert_to_tensor(x, dtype=tf.float32) / 255.    #创建tensor
...
w1 = tf.Variable(tf.random.truncated_normal([784, 256], stddev=0.1))#随机
...
lr = 1e-3   #设置学习率为：10^-3，即0.001

for epoch in range(5): # 对数据集进行迭代
    for step, (x, y) in enumerate(train_db): # for every batch
        ...
        with tf.GradientTape() as tape: # tf.Variable
            ...
            # mean: scalar
            loss = tf.reduce_mean(loss) #均值

        # compute gradients计算梯度

...
class Conv2d(module.Module):
    def __init__(self, in_channels, out_channels, kernel_size=1, stride=1, padding=0, bias=True):
        ...
        self.kernel_size = data.DataConverter.totuple(self.kernel_size)
        # 格式为: (stride_h, stride_w)
        self.stride = data.DataConverter.totuple(self.stride)
        # 格式为: (pad_h_top, pad_h_bottom, pad_w_left, pad_w_right)
        self.padding = data.DataConverter.toquaternion(self.padding)
        # 初始化权重
    ...
    def forward(self, x):
        ...
        x_cols = image.ImageConverter.im2col(x, self.kernel_size, self.stride, self.padding)
    ...
    def backward(self, accumulated_gradient):
        ...
        accumulated_gradient = image.ImageConverter.col2im(accumulated_gradient, self.storage['x'].shape, self.kernel_size, self.stride, self.padding)

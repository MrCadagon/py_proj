...
def relu_backward_2(grad_out, X):
...
def batch_norm_backward_2(grad_out, X, sum, sqrt_var, N, eps):
    # We use the formula: out = (X - mean(X)) / (sqrt(var(X)) + eps)
    # in batch norm 2d's forward. To simplify our derivation, we follow the
    # chain rule and compute the gradients as follows before accumulating
    # them all into a final grad_input.
    ...
    #  3) 'grad of out wrt X in the numerator' * 'grad of X wrt X'
    # We then rewrite the formulas to use as few extra buffers as possible
    grad_out = relu_backward(grad_out, X)
    # X = np.maximum(X, 0)
...
class BatchNorm(torch.autograd.Function):
    ...
    def backward(ctx, grad_out):
        X, = ctx.saved_tensors
        out = batch_norm_backward(grad_out, X, ctx.sum, ctx.sqrt_var, ctx.N, ctx.eps)
        return batch_norm_backward(grad_out, X, ctx.sum, ctx.sqrt_var, ctx.N, ctx.eps)


class BatchNorm_2(torch.autograd.Function):
    @staticmethod
    def forward(ctx, X, eps=1e-3):
        # Don't save keepdim'd values for backward
        sum = X.sum(dim=(0, 2, 3))
    ...
    def backward(ctx, grad_out):
        X, = ctx.saved_tensors
        out = batch_norm_backward_2(grad_out, X, ctx.sum, ctx.sqrt_var, ctx.N, ctx.eps)
        return batch_norm_backward_2(grad_out, X, ctx.sum, ctx.sqrt_var, ctx.N, ctx.eps)


class FusedConvBN2DFunction_2(torch.autograd.Function):
    @staticmethod
    ...
    def backward(ctx, grad_out):
        ...
        # test:relu
        grad_out = relu_backward_2(grad_out, bn_out)
        # test:end
...
class FusedConvBN2DFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_out):
        ...
        # test:relu
        grad_out = relu_backward(grad_out, bn_out)
        # test:end

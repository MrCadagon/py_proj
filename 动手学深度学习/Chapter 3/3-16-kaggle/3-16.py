...
def get_k_fold_data(k, i, X, y):
    assert k > 1
    fold_size = X.shape[0] // k  # 表示整除 floor
    X_train, y_train = None, None
    for j in range(k):
        idx = slice(fold_size * j, fold_size * (j + 1))
        X_now, y_now = X[idx, :], y[idx]
        if j == i:
            X_valid, y_valid = X_now, y_now
        elif X_train is None:
            X_train, y_train = X_now, y_now
        else:
            X_train = torch.cat((X_train, X_now), dim=0)
            y_train = torch.cat((y_train, y_now), dim=0)
...
def k_fold(k, X_train, y_train,
    ...
    for i in range(k):
        ...
        print('fold %s : train loss %f , test loss %f' % (i, train_ls[-1], test_ls[-1]))
